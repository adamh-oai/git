...
`--btrfs-snapshot`::
`--no-btrfs-snapshot`::
	Ask AWACS to create the worktree as a Btrfs snapshot of the current
	worktree.
	The positive form requires snapshot creation; the negative form
	disables it. These options override `worktree.btrfsSnapshot` for one
	invocation. To request automatic fallback, set that configuration to
	`auto`. The source must be clean except for ignored files and must not
	contain submodules. Git delegates the snapshot lifecycle to
	`awacs git worktree-add` when AWACS is available. See
